...
namespace UnityEngine.Rendering.Universal
{
    ...
    public static class ShadowUtils
    {
        ...
        public static bool ExtractDirectionalLightMatrix(ref CullingResults cullResults, ref ShadowData shadowData, int shadowLightIndex, int cascadeIndex, int shadowmapWidth, int shadowmapHeight, int shadowResolution, float shadowNearPlane, out Vector4 cascadeSplitDistance, out ShadowSliceData shadowSliceData)
        {
            ...
            //iMix Custom Start
            //This approach requires turning off shadows for all objects (keeping them only for the character), 
            //then using a render feature to implement custom scene shadows, with the cascade set to 1. I believe 
            //it's better to use native scene shadows and write custom code for character-specific shadows instead of forcing changes like this.
            // viewMatrix = UniversalRenderPipeline.viewMatrix;
            // shadowSliceData.viewMatrix = UniversalRenderPipeline.viewMatrix;
            // shadowSliceData.projectionMatrix = UniversalRenderPipeline.proMatrix;
