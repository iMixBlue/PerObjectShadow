...
half4 frag (Varyings IN, half FUR_OFFSET = 0) : SV_Target
{
	...
	#endif
    half NdotLorigin = saturate(dot(inputData.normalWS, mainLight.direction));
    half NdotL = step(0.5,NdotL);
    half halflambet = pow((NdotL * 0.5 + 0.5),2);
    float NdotLremapped = 1 - halflambet;
    ...
    half4 extra = SAMPLE_TEXTURE2D(_MainTex, sampler_linear_clamp_MainTex, IN.uv.xy);
